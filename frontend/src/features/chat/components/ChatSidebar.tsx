  ...
  useEffect(() => {
    const handleHistory = (payload: { boardId: string; messages: ChatMessage[] }) => {
      if (payload.boardId === boardId) {
        setMessages(payload.messages); // Backend already returns chronological order
      }
    };
          ...
          messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={cn("flex flex-col", isMe ? "items-end" : "items-start")}>
                ...
                <div
                  className={cn(
                    "px-3 py-2 rounded-xl max-w-[90%] text-sm relative group flex flex-col",
                    isMe ? "bg-primary text-primary-foreground rounded-tr-none" : "bg-muted rounded-tl-none"
                  )}
                >
                  <div className="break-words">{msg.content}</div>
                  <div className={cn(
                    "text-[10px] mt-1 text-right font-medium opacity-60",
